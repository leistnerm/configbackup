...
class ConfigLoader:
    ...
    def _resolve(self, cfg: dict[str, Any]) -> dict[str, Any]:
        ...
        pr_cfg.setdefault("repository", "auto")
...
class BackupEngine:
    ...
    def _github_repo_selector(self) -> str:
        """Return GitHub CLI's [HOST/]OWNER/REPO selector for PR operations."""
        pr_cfg = self.git_cfg.get("pull_request") or {}
        configured = str(pr_cfg.get("repository") or "auto").strip()
        if configured and configured.lower() != "auto":
            return configured

        remote = str(self.git_cfg.get("remote_name") or "origin")
        result = self._git_control_command(["remote", "get-url", remote], check=False)
        if result.returncode != 0 or not result.stdout.strip():
            raise ConfigError(
                f"Unable to determine GitHub repository from Git remote {remote!r}. "
                "Set git.pull_request.repository to OWNER/REPO (or HOST/OWNER/REPO)."
            )
        url = result.stdout.strip()

        host = ""
        repo_path = ""
        # https://host/owner/repo.git, http://..., ssh://git@host/owner/repo.git
        m = re.match(r"^(?:https?|ssh)://(?:[^@/]+@)?([^/:]+)(?::\\d+)?/(.+)$", url, re.IGNORECASE)
        if m:
        ...
        if not host or len(parts) < 2:
            raise ConfigError(
                f"Git remote {remote!r} does not look like a GitHub remote: {url!r}. "
                "Set git.pull_request.repository explicitly to OWNER/REPO (or HOST/OWNER/REPO)."
            )
        owner_repo = "/".join(parts[-2:])
        return owner_repo if host.lower() == "github.com" else f"{host}/{owner_repo}"
    ...
    def _gh_command(self, args: list[str], *, check: bool = True) -> subprocess.CompletedProcess[str]:
        ...
        env = dict(os.environ)
        # PR commands must never depend on the caller's current directory or on
        # gh successfully inferring a repo from a temporary linked worktree.
        if args and args[0] == "pr":
            env["GH_REPO"] = self._github_repo_selector()
        completed = subprocess.run(
            ["gh", *args], cwd=str(cwd) if cwd else None, env=env, text=True,
