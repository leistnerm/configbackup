...
class BackupEngine:
    def __init__(self, cfg: dict[str, Any], *, dry_run: bool = False, prune_only: bool = False):
        ...
        if self._uses_git:
            self.git_repo = Path(str(self.git_cfg["repository"])).expanduser().absolute()
    ...
    def _prepare_git_repo(self) -> None:
        ...
        if status:
            raise ConfigError(
                ...
                "ConfigBackup requires a clean working tree so it cannot accidentally commit or discard unrelated changes."
            )
        ...
        if remote_url:
            ...
            current_remote = self._git_command(["remote", "get-url", remote], check=False)
            if current_remote.returncode != 0:
                self._git_command(["remote", "add", remote, remote_url])
    ...
    def _store_one_file(self, task: dict[str, Any], src: Path, logical: Path, source_display: str | None = None) -> str:
        ...
        changed_statuses = [x for x in statuses if x != "unchanged"]
        if not changed_statuses:
            ...
            combined = "changed"
            self.stats.changed += 1
        stored_count = sum(1 for x in statuses if x != "unchanged")
    ...
    def _finalize_git_repo(self) -> None:
        ...
        message = message_template.format(hostname=self.hostname, run_id=self.run_id, date=self.start_time.strftime("%Y-%m-%d"))
        ...
        if self.git_cfg.get("push"):
            remote = str(self.git_cfg.get("remote_name") or "origin")
            branch = str(self.git_cfg.get("branch") or "main")
            self._git_command(["push", remote, branch])
            self.logger.info("Git snapshot pushed to %s/%s", remote, branch)

    def _rollback_git_repo(self) -> None:
        if self.git_repo is None or self.dry_run or not (self.git_repo / ".git").exists():
            return
        has_head = self._git_command(["rev-parse", "--verify", "HEAD"], check=False).returncode == 0
        if has_head:
